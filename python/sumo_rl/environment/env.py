...
class SumoEnvironment(gym.Env):
    ...
    def __init__(
        ...
        if LIBSUMO:
            traci.start([sumolib.checkBinary("sumo"), "-n", self._net,'--collision.stoptime','10'])  # Start only to retrieve traffic light information
            conn = traci
        else:
            traci.start([sumolib.checkBinary("sumo"), "-n", self._net,'--collision.stoptime','10'], label="init_connection" + self.label)
